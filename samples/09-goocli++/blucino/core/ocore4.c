...
//		     p->current);
  int id = p->led_gpio_pin - 16;
	BL_ob o = {BL_GOOSRV,BL_LET,id,NULL};
  bl_event(&o,p->current);                                            //@@@4.9
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
static void prov_complete(uint16_t net_idx, uint16_t addr)
{
	...
	BL_ob o = {BL_MESH, BL_PRV, 0, NULL};                               //@@@3.4
	bl_event(&o,1);
}
...
{
	BL_ob o = {BL_BUTTON, BL_SET, sw.sw_num+1, NULL};   // button ID = sw_num+1
	bl_event(&o,sw.onoff_state);
}
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
//==============================================================================
  ...
  {
    bl_log2(3,"@core.led",o->id,value);                                //@@@3.7
  	if (o->id >= 1 && o->id <= 4)
  	{
      struct onoff_state *p = onoff_state + (o->id-1);
  		//p->current = value;
      gpio_pin_set(p->led_device, p->led_gpio_pin, value);
  	}
  }
