...
// - [RESET:#INC due]      // trigger to emit a [RESET:INC] event with 5s due
  ...
  static int startup(BL_ob *o, int val)     // public module interface
  {
    ...
    switch (bl_id(o))
    {
      ...
      case BL_ID(_RESET,_INC_):             // [RESET:#INC] entry point
        count = bl_out(o,T_STARTUP,out);    // set startup reset interval
        return 0;                           // OK
      ...
      case BL_ID(_BUTTON,PRESS_):           // button press during startup
        ...
        {
          BL_ob oo = {_RESET,_INC_,0,NULL};
          startup(&oo,0);                   // post [RESET:#INC] to STARTUP
        }
        return 0;                           // OK
...
//==============================================================================
  ...
  static int init(BL_ob *o, int val)   // init all modules
  {
    ...
    bl_init(startup,bl_down);          // init startup module, output down
    return 0;                          // OK
  }
