...
#include "transition.h"
  ...
  #define _DUE_  BL_HASH(DUE_)         // hashed symbol #DUE
...
static void reset_counter_timer_handler(struct k_timer *dummy)
{
  ...
  #if MIGRATION_STEP5
    BL_ob oo = {_RESET,_DUE_};         // [RESET:#DUE]
    bl_core(&oo,0);                    // provide [HDL:#DUE] for output
  #endif
}
  ...
  int bl_core(BL_ob *o, int val)
  {
    ...
    switch (BL_ID(o->cl,o->op))
    {
      ...
      case BL_ID(_RESET,_DUE_):        // [RESET:#DUE] reset timer is due
        return bl_out(o,val,out);      // output [RESET:DUE] to subscriber
