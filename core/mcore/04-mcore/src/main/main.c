...
//==============================================================================
  ...
  #define _INC_ BL_HASH(INC_)          // hashed symbol #INC
  ...
  static int startup(BL_ob *o, int val)     // public module interface
  {
    ...
    switch (bl_id(o))
    {
      case BL_ID(_SYS,INIT_):               // [SYS:INIT <out>] init module
      {
        ...
        BL_ob oo = {_RESET,_INC_,0,0};
        startup(&oo,0);                     // provide [RESET:#INC] for output
        ...
        else
        {
          ...
          BL_ob oo = {_RESET,PRV_,0,NULL};  // [RESET:PRV]
          return out(&oo,0);                // unprovision node
        }
      }

      case BL_ID(_RESET,_INC_):             // provide [RESET:#INC] for output
      {
        count = bl_out(o,T_STARTUP,out);    // startup reset int'val
        return 0;                           // OK
      }
...
//==============================================================================
  ...
  static int init(BL_ob *o, int val)   // init all modules
  {
    ...
    bl_init(startup,bl_down);          // init startup module
    return 0;                          // OK
  }
